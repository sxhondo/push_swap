/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   push_swap.c                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sxhondo <w13cho@gmail.com>                 +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/01 13:50:48 by sxhondo           #+#    #+#             */
/*   Updated: 2019/10/01 13:50:49 by sxhondo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

/*
int				brute_force(t_list **a, t_list **b)
{
	int 		top_a;
	int 		top_b;
	unsigned 	i, j;
	unsigned 	ops;

	ops = 0;
	if (!*a)
		return (1);
	top_a = *a ? *((int *)(*a)->content) : 0;
	top_b = *b ? *((int *)(*b)->content) : 0;
	if (top_a > top_b && ++ops)
		do_push(a, b, 'b');
	else
	{
		i = 0;
		while (top_a < top_b)
		{
			do_push(a, b, 'a');
			ops++;
			if ((*b))
				top_b = *((int *)(*b)->content);
			else
				top_b = 0;
			i++;
		}
		j = i;
		while (i-- && ++ops)
			do_rot(a);
		do_push(a, b, 'b');
		ops++;
		while (i++ < j && ++ops)
			do_rev_rot(a);
	}
	print_stacks(a, b);
	brute_force(a, b);
	return (1);
}
*/

int 			is_sorted(t_list **stack)
{
	t_list		*tmp;

	tmp = *stack;
	if (!tmp)
		return (1);
	while (tmp->next)
	{
		if (*((int *)tmp->content) < *((int *)tmp->next->content))
			tmp = tmp->next;
		else
			return (0);
	}
	return (1);
}

int 			is_back_sorted(t_list **stack)
{
	t_list		*tmp;

	tmp = *stack;
	while (tmp->next)
	{
		if (*((int *)tmp->content) > *((int *)tmp->next->content))
			tmp = tmp->next;
		else
			return (0);
	}
	return (1);
}


int 			find_index(t_list **stack, int min)
{
	t_list		*tmp;
	int 		i;

	i = 0;
	tmp = *stack;
	while (tmp)
	{
		if (*((int *)tmp->content) == min)
			return (i);
		i++;
		tmp = tmp->next;
	}
	return (-1);
}

int 			get_value_for_index(t_list **stack, int index)
{
	t_list		*tmp;

	tmp = *stack;
	while (index)
	{
		tmp = tmp->next;
		index--;
	}
	return (*((int *)tmp->content));
}

void			value_to_top(t_list **stack, int value)
{
	int 		llen;
	int 		value_i;

	llen = ft_lstlen(stack);
	value_i = find_index(stack, value);
	if (value_i < llen / 2)
		while (value_i--)
			do_rot(stack);
	else
		while (value_i++ <= llen - 1)
			do_rev_rot(stack);
}


int 			check_max(t_list **stack, int k_val)
{
	t_list		*tmp;

	tmp = *stack;
	while (tmp)
	{
		if (*((int *)tmp->content) > k_val)
			return (0);
		tmp = tmp->next;
	}
	return (1);
}

int				find_min_max(t_list **stack, int mix) /* 0 - min, 1 - max */
{
	t_list		*i;
	t_list		*j;
	int 		curr;
	int 		t;

	i = *stack;
	while (i)
	{
		t = 0;
		j = *stack;
		curr = *((int *)i->content);
		while (j)
		{
			if (!mix && curr <= *((int *)j->content))
				t++;
			if (mix && curr >= *((int *)j->content))
				t++;
			j = j->next;
		}
		if (t == ft_lstlen(stack))
			return (curr);
		i = i->next;
	}
	return (-1);
}

//void			make_pairs(t_list **a, t_list **b)
//{
//	int 		min_len = ft_lstlen(a) > ft_lstlen(b) ?
//			ft_lstlen(b) : ft_lstlen(a);
//
//	while (min_len--)
//	{
//		int 		top_a = *((int *)(*a)->content);
//		int 		top_b = *((int *)(*b)->content);
//		if (top_a > top_b)
//		{
//			do_push(a, b, 'b');
//			do_rot(b);
//			do_push(a, b, 'a');
//			do_rot(a);
//			print_stacks(a, b);
//		}
//		else
//		{
//			do_rot(a);
//			do_rot(b);
//		}
//		print_stacks(a, b);
//	}
//}
//
//void			make_groups(t_list **a, t_list **b)
//{
//
//
//	int i = 5;
//	while (i--)
//	{
//		int 		top_a = *((int *)(*a)->content);
//		int 		top_a_next = *((int *)(*a)->next->content);
//		int 		top_b = *((int *)(*b)->content);
//		int 		top_b_next = *((int *)(*b)->next->content);
//
//		if (top_a > top_a_next)
//		{
//			do_swap(a);
//			do_rot(a);
//			print_stacks(a, b);
//		}
//		if (top_a < top_b_next)
//		{
//			do_rot(a);
//			print_stacks(a, b);
//		}
//	}
//	print_stacks(a, b);
//}
//
//void			merge_sort(t_list **a, t_list **b)
//{
//	int 		med = ft_lstlen(a) / 2;
//
//	while (med--)
//		do_push(a, b, 'b');
//	print_stacks(a, b);
//
//	make_pairs(a, b);
//	make_groups(a, b);
//}

int				partition(t_list **a, t_list **b,
						int m_index, int right, char stack)
{
	int 		k_val;
	int 		top;
	int 		i;

	i = 0;
	top = stack == 'a' ? *((int *)(*a)->content) : *((int *)(*b)->content);
	k_val = get_value_for_index(stack == 'a' ? a : b, m_index);

	while (right--)
	{
		top = stack == 'a' ? *((int *)(*a)->content) : *((int *)(*b)->content);
		if (top <= k_val)
		{
			do_push(a, b, stack == 'a' ? 'b' : 'a');
			i++;
			continue;
		}
		do_rot(stack == 'a' ? a : b);
	}
	return (i);
}

void		quick_sort(t_list **a, t_list **b,
						   int n, char stack)
{
	if (n == 0)
		return ;

	int top_half_len = 0;
	int tmp = 0;
	int m_index = n - 1;

	print_stacks(a, b);

	top_half_len = partition(a, b, m_index, n, stack);
	print_stacks(a, b);

	tmp =  n - top_half_len;
	while (tmp--)
		do_rev_rot(stack == 'a' ? a : b);

	tmp = top_half_len;
	while (tmp--)
		do_push(a, b, stack);


	print_stacks(a, b);
	if (!is_sorted(a))
		quick_sort(a, b, top_half_len - 1, 'a');
	if (!is_sorted(b))
		quick_sort(a, b, ft_lstlen(b), 'b');
	tmp = ft_lstlen(b);
}

void 			sort_stacks(int *nums, unsigned arg_am)
{
	t_list		*a;
	t_list		*b;

	b = NULL;
	a = fill_a(nums, arg_am);
//	ops = brute_force(&a, &b);


//	for (int med = ft_lstlen(&a) / 2; med; med--)
//		do_push(&a, &b, 'b');
	partition(&a, &b, ft_lstlen(&a) - 1, ft_lstlen(&a), 'a');

	print_stacks(&a, &b);
	quick_sort(&a, &b, ft_lstlen(&a), 'a');

	print_stacks(&a, &b);
	for (int i = ft_lstlen(&b); i; i--)
		{do_rev_rot(&b); do_push(&a, &b, 'a');}

	print_stacks(&a, &b);
	free_stacks(&a, &b);
}
